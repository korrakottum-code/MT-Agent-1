...
// TASK = มีคนสั่งและมีคนรับปาก เท่านั้น อย่างอื่นที่น่าจำเป็น NOTE ซึ่งไม่ใช่คิวงาน
// เส้นแบ่งนี้มาจากเจ้าของงานเอง (9 ก.ย. 2569): "สั่งแล้วมีคนรับ" = งาน · "ถามแล้วตอบ" = แค่รู้ไว้
const EVENT_TYPES = ["TASK", "NOTE", "DECISION", "DEADLINE"];
        ...
        items: {
          ...
          properties: {
            type: {
              ...
              enum: ["TASK", "NOTE"],
              description:
                "TASK = มีคนสั่งงานให้คนใดคนหนึ่งชัดเจน และคนนั้นรับปากแล้ว · " +
                "NOTE = อย่างอื่นทั้งหมดที่ควรจำไว้ ทั้งข้อตกลง ข้อมูล การยืนยัน และแผนที่ยังไม่มีคนรับ",
            },
  ...
  for (const g of groups ?? []) {
    try {
      ...
      const response: any = await createMessage(spec, {
        max_tokens: 2000,
        system:
          `คุณคือแงว อ่านบทสนทนาในกลุ่มงานแล้วบันทึกสิ่งที่ควรจำไว้ แบ่งเป็นสองแบบเท่านั้น\n\n` +
          `TASK — งานที่ "มีคนสั่ง และมีคนรับ" ครบทั้งสองฝั่งในบทสนทนา\n` +
          `- ต้องเห็นชัดว่าใครสั่ง สั่งให้ใคร ให้ทำอะไร และคนนั้นตอบรับแล้ว (เช่น "ครับ" "โอเค" "ได้ค่ะ" "รับทราบ" "เดี๋ยวจัดการ")\n` +
          `- ตัวอย่างที่เป็น TASK: "ทอง พรุ่งนี้ขึ้นแอดภาพ A นะ" แล้วทองตอบ "โอเคครับ"\n` +
          `- ถ้าสั่งแล้วยังไม่มีใครตอบรับ ให้เป็น NOTE ไม่ใช่ TASK\n` +
          `- ถ้าไม่รู้ว่าใครต้องทำ ให้เป็น NOTE ไม่ใช่ TASK\n\n` +
          `NOTE — ทุกอย่างที่เหลือที่ควรจำไว้ใช้ตอบคำถามภายหลัง แต่ไม่ใช่คิวงานของใคร\n` +
          `- คำถามที่ได้คำตอบแล้ว เช่น "พรุ่งนี้เราขึ้นแอดใช่มั้ย" แล้วทองตอบ "ใช่" — อันนี้คือรู้ไว้เฉย ๆ ห้ามทำเป็น TASK\n` +
          `- ข้อตกลง มติ ตัวเลข กำหนดการ สถานะงาน การแจ้งให้ทราบ การลา\n` +
          `- แผนหรือไอเดียที่ยังไม่มีใครรับปาก\n` +
          `- การปรึกษากัน ถกกัน เสนอความเห็น หรือชั่งใจว่าจะเอาทางไหนดี — ต่อให้คุยกันยาวและจบด้วยความเห็นตรงกัน ก็ยังเป็น NOTE\n` +
          `  ตราบใดที่ไม่มีประโยคที่สั่งให้ใครไปทำ และไม่มีคนนั้นรับปาก มันไม่ใช่งาน\n` +
          `- หัวหน้าพูดว่า "ที่มองเห็นหลัก ๆ คือ..." หรือ "น่าจะทำแบบนี้" คือการวางแนวคิด ไม่ใช่การมอบหมาย\n\n` +
          `เกณฑ์ที่ใช้กับทั้งสองแบบ:\n` +
          `- จับเฉพาะที่ชัดเจนพอจะเอาไปใช้ต่อได้ ห้ามเดาหรือแต่งเติมสิ่งที่ไม่มีใครพูด\n` +
          `- ตัวเลขและวันที่ ต้องมีคนพิมพ์ไว้จริงเท่านั้น ห้ามคำนวณเองหรืออนุมานเอง\n` +
          `- ข้ามการทักทาย คุยเล่น แซว บ่น และคำถามที่ยังไม่มีใครตอบ\n` +
          ...
          `- due_at ใส่เฉพาะเมื่อมีคนระบุกำหนดไว้ชัดเจน ถ้าไม่มีให้เว้นว่าง อย่าเดาวันเอง\n` +
